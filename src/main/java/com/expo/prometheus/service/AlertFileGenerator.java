...
public class AlertFileGenerator {
    ...
    private static final String ALERT_FILE_NAME = "alertmanager.yml";
    ...
    String theLocalRulesFile=ALERT_FILE_NAME;
    ...
    public void generateConfigFile() {
        ...
        String alertFilePath = alertManagerConfigPath+"/"+ALERT_FILE_NAME;

        try {
            FileWriter writer = new FileWriter(alertFilePath);
    ...
    public boolean modifyInstanceInAlertFile(String oldInstance, String newInstance) {
        try {
            String alertFilePath2 = alertManagerConfigPath+"/"+ALERT_FILE_NAME;
            Path alertFilePath = Path.of( alertFilePath2);
                ...
                if (routes != null) {
                    for (Map<String, Object> route : routes) {
                        ...
                        if (match != null) {
                            ...
                            if (routeInstance != null && routeInstance.equals(oldInstance)) {
                                // Modify the instance IP address in the 'instance' field of the 'match' section
                                match.put("instance", newInstance);
                            }
                        }
                    }
                }
            ...
            System.out.println("Instance IP address in alert file modified successfully.");
         //   this.prometheusAlertService.pushRuleFile(theLocalRulesFile,alertManagerConfigPath);
            return true;
        } catch (IOException e) {
            System.err.println("Failed to modify the instance IP address in the alert file: " + e.getMessage());
            return false;
        }
    }
    ...
    public void addRoute(String alertName, String receiverName, String instance, List<String> receiverEmails) throws IOException {
        ...
        try {
            Path alertFilePath = Path.of( alertManagerConfigPath+"/",ALERT_FILE_NAME);
    ...
    private void writeYamlConfig(Map<String, Object> config, Path filePath) throws IOException {
        ...
        options.setDefaultFlowStyle(DumperOptions.FlowStyle.BLOCK); // l block pour more readable YAML .
        ...
        try {
            ...
            if (Files.exists(alertFilePath)) {
                ...
                if (config != null) {
                    ...
                    FileWriter writer = new FileWriter(alertFilePath.toFile()); //writing l config l jdida fl write wl alertFilePath.toFile() is a file object
                    yaml.dump(config, writer); // serialisation taa config fl writer
                    writer.close();
    ...
    private List<MailNotifInfo> parseReceivers(Object receiversObject, String instance, boolean state) {
        ...
        if (receiversObject instanceof List) {
            ...
            for (Map<String, Object> receiverMap : receiversList) {
                ...
                receivers.add(new MailNotifInfo(name, instance, emails, state, receiver));
            }
        }
